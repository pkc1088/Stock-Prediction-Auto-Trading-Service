...
def holding():
    try:
        stock_dict = make_holding(access_token)
        holdings_list = [
            HoldingItemResponse(
                name=item["name"],
                symbol=item["symbol"],
...
def trade(stock_req : TradeRequest):
    try : 
        ...
        t = threading.Thread(target=auto_trade, args=(access_token, symbol, predictedPrice), daemon=True)
        t.start()
...
def predict(stock_req: StockRequest):
    ...
    current_price = get_current_price(access_token, stock_req.symbol)# make_stock(stock_req.symbol) # 1
    prices = make_price_point(stock_req.symbol) # get_price_history(stock_req.symbol, target_date) # 2
    ...
    short_text = full_news_text[:10000]
    print(f"[INFO] 수집한 뉴스 본문 전체 길이: {len(full_news_text)}자")
    # Gemini 
    score, explanation = analyze_sentiment_with_gemini(news_text=short_text, stock_name=stock_req.name)
